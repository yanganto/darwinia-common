...
fn proxy_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Democracy::activate_proxy(Origin::signed(1), 10),
			Error::<Test>::NotOpen
		);
		...
		assert_noop!(
			Democracy::activate_proxy(Origin::signed(2), 10),
			Error::<Test>::WrongOpen
		);
		...
		assert_noop!(
			Democracy::deactivate_proxy(Origin::signed(2), 10),
			Error::<Test>::WrongProxy
		);
...
fn delegation_and_undelegation_should_work_with_proxy() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::proxy_delegate(
			Origin::signed(10),
			2,
			Conviction::None,
			10
